...
import type { ChatChannel, ChatMessage, APINotification } from '../types';
...
const MessagesPage: React.FC = () => {
  ...
  const loadChannelMessages = useCallback(
    async (channelId: number): Promise<ChatMessage[] | null> => {
      try {
  ...
  const setChannels = useCallback(
    (value: ChatChannel[] | ((prev: ChatChannel[]) => ChatChannel[])) => {
      _setChannels((prev) => {
    ...
    setChannels((prev) =>
      prev.map((channel) => {
        if (channel.channel_id === channelId) {
          ...
          console.log(
            `更新频道 ${channel.name} 已读ID: ${channel.last_read_id} -> ${newLastReadId}`
          );
          return {
            ...channel,
  ...
  // 使用通知系统
  const { notifications, unreadCount, markAsRead, refresh, removeNotificationByObject } =
  ...
  const { isConnected: chatConnected } = useWebSocketNotifications({
    ...
    onNewMessage: (message) => {
      ...
      if (shouldAddToCurrentChannel) {
        ...
        console.log(
          '消息不属于当前频道，当前频道:',
          currentSelectedChannel.channel_id,
          '消息频道:',
          message.channel_id
        );
      } else {
        ...
        console.log(
          '频道列表:',
          channelsRef.current.map((ch) => ({ id: ch.channel_id, name: ch.name }))
        );
  ...
  useEffect(() => {
    ...
    const loadChannels = async () => {
      try {
        ...
        console.log(
          '排序后的频道列表:',
          sortedChannels.map((ch: ChatChannel) => ({
            id: ch.channel_id,
            name: ch.name,
            type: ch.type,
          }))
        ...
        if (!selectedChannel && sortedChannels.length > 0) {
          // 查找 osu! 频道
          const osuChannel = sortedChannels.find(
            (ch) =>
  ...
  const selectChannelAndAddMessage = async (channel: ChatChannel, newMessage: ChatMessage) => {
    ...
    try {
      ...
      if (channelMessages && channelMessages.length > 0) {
        ...
        // 检查新消息是否已经在历史消息中
        const messageExists = channelMessages.find(
          (m: ChatMessage) => m.message_id === newMessage.message_id
        );
        ...
        if (channel.type === 'PM') {
          try {
            const relatedPmNotifications = notifications.filter(
              (n) => n.name === 'channel_message' && n.object_id === channel.channel_id.toString()
            );
            if (relatedPmNotifications.some((n) => !n.is_read)) {
              console.log(
                '私聊频道打开，自动标记通知为已读 (addMessage path):',
                relatedPmNotifications.map((n) => n.id)
              );
      ...
      if (channelMessages && channelMessages.length > 0) {
        ...
        if (channel.type === 'PM') {
          try {
            ...
            if (relatedPmNotifications.some((n) => !n.is_read)) {
              console.log(
                '私聊频道打开，自动标记通知为已读 (selectChannel path):',
                relatedPmNotifications.map((n) => n.id)
              );
              await removeNotificationByObject(channel.channel_id.toString(), 'channel');
            }
  ...
  const addMessageToList = useCallback((message: ChatMessage, source: 'api' | 'websocket') => {
    console.log(
      `添加消息(${source}): ID=${message.message_id}, 频道ID=${message.channel_id}, 发送者=${message.sender_id}, 内容="${message.content.substring(0, 30)}"`
    );
    ...
    if (!currentChannel || message.channel_id !== currentChannel.channel_id) {
      console.log(
        `消息不属于当前频道，跳过添加。当前频道: ${currentChannel?.channel_id || 'null'}, 消息频道: ${message.channel_id}`
      );
      return;
    }
    ...
    setMessages((prev) => {
      ...
      console.log(
        `成功添加消息: ${message.message_id}, 当前消息列表长度: ${prev.length} -> ${prev.length + 1}`
      );
  ...
  const sendMessage = async (messageText: string) => {
    ...
    try {
      const message = await chatAPI.sendMessage(selectedChannel.channel_id, messageText.trim());
  ...
  useEffect(() => {
    const container =
      scrollContainerRef.current || document.querySelector('#chat-message-scroll-container');
    if (!container) return;
  ...
  useEffect(() => {
    ...
    if (userIdsToFetch.size > 0) {
      apiCache
        .getUsers(Array.from(userIdsToFetch))
        .then(() => {
  ...
  const autoMarkPrivateMessagesAsRead = async (notification: APINotification) => {
    ...
    try {
      ...
      const matchingMessages = channelMessages.filter((message: ChatMessage) => {
        ...
        const isMatch =
          exactMatch || contentIncludesTitle || titleIncludesContent || similarityMatch;
          ...
          console.log('找到匹配的消息:', {
            ...
            matchType: exactMatch
              ...
              : contentIncludesTitle
                ...
                : titleIncludesContent
                  ? 'title_includes_content'
                  : 'similarity',
            similarity: similarity.toFixed(2),
          });
        }
      ...
      if (uniqueMessages.length > 0) {
        ...
        console.log(
          `找到 ${uniqueMessages.length} 条唯一匹配消息（原始 ${matchingMessages.length} 条），将标记到消息ID: ${maxMessageIdToMark}`
        );
        ...
        shouldMarkAsRead = true;
        console.log(
          `用户正在查看频道 ${channelId}，标记所有消息为已读，最大消息ID: ${maxMessageIdToMark}`
        );
      }
      ...
      if (shouldMarkAsRead && maxMessageIdToMark > 0) {
        ...
        if (maxMessageIdToMark > currentLastReadId) {
          console.log(
            `准备标记频道 ${channelId} 消息 ${maxMessageIdToMark} 为已读 (当前已读: ${currentLastReadId})`
          );
          ...
          console.log(
            `消息 ${maxMessageIdToMark} 已经被标记为已读 (当前已读: ${currentLastReadId})`
          );
        }
      } else {
  ...
  const batchMarkPrivateNotificationsAsRead = async () => {
    ...
    const privateNotifications = notifications.filter(
      (notification) =>
        notification.name === 'channel_message' && notification.details?.type === 'pm'
    );
    ...
    console.log(
      `通知去重: 原始 ${privateNotifications.length} 个，去重后 ${deduplicatedNotifications.length} 个`
    );
  ...
  const getNotificationTitle = useCallback(
    (notification: APINotification): string => {
        ...
        case 'channel_message':
          if (notification.details?.type === 'pm') {
            ...
            if (
              ...
              messageContent.trim() &&
              !placeholders.includes(messageContent)
            ) {
                          ...
                          >
                            <div className="flex items-start space-x-3">
                              <div className="flex-shrink-0">
                                ...
                                hasUserInfoInCache(notification.source_user_id) ? (
                                  <img
                                  ...
                                  hasUserInfoInCache(notification.source_user_id) && (
                                    <div className="flex items-center space-x-2 mt-2">
                                      ...
                                      <span className="text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded">
                                        {(() => {
                                          // 从缓存获取用户名
                                          const cachedUser = getUserInfoFromCache(
                                            notification.source_user_id!
                                          );
                                          return (
                                            cachedUser?.username ||
                                            t('messages.sidebar.unknownUser')
                                          );
                                        })()}
                                      </span>
                                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 min-h-0">
              {messages.map((message, index) => {
                ...
                if (prevMessage && prevMessage.sender_id === message.sender_id) {
                  ...
                  console.log(
                    `消息分组检查: ${message.message_id}, 时间差: ${timeDiff}ms, 是否分组: ${isGrouped}`
                  );
                }

                return (
                  <div
                    key={`message-${message.message_id}-${message.channel_id}-${index}`}
