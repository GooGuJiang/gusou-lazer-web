...
import type { GameMode, TopUsersResponse, CountryResponse, TabType, RankingType } from '../types';
...
const RankingsPage: React.FC = () => {
  ...
  // 获取可用国家列表
  const { countries: availableCountries, isLoading: isLoadingCountries } =
    useAvailableCountries(selectedMode);
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 sm:py-8">
        ...
        <div className="-mx-4 sm:mx-0 sm:bg-card sm:rounded-xl sm:shadow-sm sm:border-card sm:p-6">
          ...
          {!isLoading && (
            <PaginationControls
              total={
                selectedTab === 'users' ? userRankings?.total || 0 : countryRankings?.total || 0
              }
              currentPage={currentPage}
