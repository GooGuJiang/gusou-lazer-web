...
const BBCodeEditor: React.FC<BBCodeEditorProps> = ({
  ...
  const insertBBCode = useCallback(
    (openTag: string, closeTag: string, defaultContent: string = '') => {
      ...
      const newText =
        value.substring(0, start) + openTag + contentToWrap + closeTag + value.substring(end);
        ...
        if (textarea && wasActive) {
          try {
            ...
            if (selectedText) {
              ...
              textarea.setSelectionRange(
                start + openTag.length,
                start + openTag.length + defaultContent.length
              );
            }
          } catch (error) {
            // 忽略可能的焦点设置错误
  ...
  const tools: BBCodeTool[] = [
    ...
    },
    {
      ...
      action: () =>
        insertBBCode('[strike]', '[/strike]', t('profile.bbcodeEditor.insertText.strikethrough')),
    },
    {
      icon: FaPalette,
      tooltip: t('profile.bbcodeEditor.toolbar.color'),
      action: () =>
        insertBBCode('[color=red]', '[/color]', t('profile.bbcodeEditor.insertText.colorText')),
    },
    {
      icon: FaFont,
      tooltip: t('profile.bbcodeEditor.toolbar.fontSize'),
      action: () =>
        insertBBCode('[size=100]', '[/size]', t('profile.bbcodeEditor.insertText.text')),
    },
    ...
    },
    {
      ...
      action: () =>
        insertBBCode(
          '[url=',
          `]${t('profile.bbcodeEditor.insertText.linkText')}[/url]`,
          'https://example.com'
        ),
    },
    {
      ...
      action: () =>
        insertBBCode(
          '[profile=',
          `]${t('profile.bbcodeEditor.insertText.username')}[/profile]`,
          '123456'
        ),
    },
    {
      ...
      action: () =>
        insertBBCode(
          '[email=',
          `]${t('profile.bbcodeEditor.insertText.emailLink')}[/email]`,
          'example@example.com'
        ),
    },
    {
    ...
    // 结构化内容
    {
      ...
      action: () =>
        insertBBCode('[quote]', '[/quote]', t('profile.bbcodeEditor.insertText.quoteContent')),
    },
    {
      icon: FaCode,
      tooltip: t('profile.bbcodeEditor.toolbar.code'),
      action: () =>
        insertBBCode('[code]', '[/code]', t('profile.bbcodeEditor.insertText.codeContent')),
    },
    {
      icon: FaList,
      ...
      action: () =>
        insertBBCode(
          ...
          t('profile.bbcodeEditor.insertText.item1')
        ),
    },
    {
      icon: FaBox,
      ...
      action: () =>
        insertBBCode(
          `[box=${t('profile.bbcodeEditor.insertText.title')}]`,
          '[/box]',
          t('profile.bbcodeEditor.insertText.collapsibleContent')
        ),
    },
    {
      icon: FaMask,
      ...
      action: () =>
        insertBBCode(
          '[spoiler]',
          '[/spoiler]',
          t('profile.bbcodeEditor.insertText.spoilerContent')
        ),
    },
    {
      icon: FaAlignCenter,
      ...
      action: () =>
        insertBBCode('[centre]', '[/centre]', t('profile.bbcodeEditor.insertText.centerText')),
    },
    {
      icon: FaHeading,
      tooltip: t('profile.bbcodeEditor.toolbar.heading'),
      action: () =>
        insertBBCode('[heading]', '[/heading]', t('profile.bbcodeEditor.insertText.headingText')),
    },
    {
      icon: FaExclamationTriangle,
      tooltip: t('profile.bbcodeEditor.toolbar.notice'),
      action: () =>
        insertBBCode('[notice]', '[/notice]', t('profile.bbcodeEditor.insertText.importantNotice')),
    },
  ];
  ...
  const insertSize = useCallback(
    (size: number) => {
      insertBBCode(
        `[size=${size}]`,
        '[/size]',
        `${size}px ${t('profile.bbcodeEditor.insertText.text')}`
      );
    },
    [insertBBCode, t]
  );
  ...
  return (
    <div className={`${className}`}>
      ...
      <div className="border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden bg-card">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            >
              <option value="" disabled>
                {t('profile.bbcodeEditor.fontSize.label')}
              </option>
              ...
              {isPreviewMode ? (
                ...
                <>
                  ...
                  <span className="hidden sm:inline">
                    {t('profile.bbcodeEditor.modes.preview')}
                  </span>
                </>
              )}
            </button>
              ...
              {validationLoading ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <span className="ml-2 text-sm text-gray-500 dark:text-gray-400">
                    {t('profile.bbcodeEditor.preview.generating')}
                  </span>
                </div>
              ) : validationError ? (
                ...
                <div className="text-center py-8 text-gray-500 dark:text-gray-400 text-sm">
                  {value.trim()
                    ? t('profile.bbcodeEditor.preview.generateFailed')
                    : t('profile.bbcodeEditor.preview.noContent')}
                </div>
              )}
            </div>
          ) : (
          ...
          {!isPreviewMode && (
            <div className="absolute top-2 right-2 flex items-center gap-2">
              ...
              {validationResult && !validationLoading && (
                ...
                >
                  {validationResult.valid
                    ? `✓ ${t('profile.bbcodeEditor.validation.syntaxCorrect')}`
                    : `✗ ${validationResult.errors.length}${t('profile.bbcodeEditor.validation.errors')}`}
                </div>
              )}
            </div>
          )}
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 px-2 py-1 bg-gray-50 dark:bg-gray-700/30">
          <details className="text-xs text-gray-600 dark:text-gray-400">
            ...
            <div className="mt-1 pb-1 grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-0.5 text-xs">
              <div>
                <strong>[b]{t('profile.bbcodeEditor.help.examples.bold')}[/b]</strong>
              </div>
              <div>
                <em>[i]{t('profile.bbcodeEditor.help.examples.italic')}[/i]</em>
              </div>
              <div>
                <u>[u]{t('profile.bbcodeEditor.help.examples.underline')}[/u]</u>
              </div>
              <div>
                <del>[s]{t('profile.bbcodeEditor.help.examples.strikethrough')}[/s]</del>
              </div>
              ...
              <div>
                [url={t('profile.bbcodeEditor.help.examples.link')}]
                {t('profile.bbcodeEditor.help.examples.text')}[/url]
              </div>
              ...
              <div className="md:col-span-1 col-span-2">
                [list][*]{t('profile.bbcodeEditor.help.examples.item1')}[*]
                {t('profile.bbcodeEditor.help.examples.item2')}[/list]
              </div>
            </div>
          </details>
        </div>
        ...
        {/* BBCode帮助模态框 */}
        <BBCodeHelpModal isOpen={isHelpModalOpen} onClose={() => setIsHelpModalOpen(false)} />
      </div>
    </div>
  );
};
