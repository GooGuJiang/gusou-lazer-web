...
const ContentContainer: React.FC<ContentContainerProps> = ({
  ...
  return (
    <div
      className={`relative transition-[margin-bottom] duration-300 ease-in-out ${isExpanded ? 'mb-10' : ''} ${className}`}
    >
      {/* 内容区域 */}
      <div
        ref={contentRef}
      ...
      <div className="absolute bottom-0 left-0 right-0">
        ...
        <div
          className={`h-16 bg-gradient-to-t from-white dark:from-gray-900 to-transparent pointer-events-none
        ...
        {showButton && showExpandButton && (
          <div
            className={`absolute justify-self-center text-center transition-[bottom] duration-200 z-10 ${
          ...
          >
            ...
            >
              {/* 图标旋转动画 */}
              <svg
                className={`w-4 h-4 mr-1 transition-transform duration-300 ${isExpanded ? '' : 'rotate-180'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
