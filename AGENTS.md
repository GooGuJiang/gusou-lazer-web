...
`gusou-lazer-web` 是面向 g0v0 / osu! lazer 生态的 Next.js Web 客户端。公开页面由 App Router 服务端渲染，现有复杂交互页面通过 React Router 兼容层渐进迁移。
...
本文件汇总当前仓库的开发约束。实现细节以现有代码以及 `package.json`、TypeScript、ESLint、Prettier 和 Next.js 配置为准。
...
| 构建与开发 | Next.js 16 App Router、pnpm、ESM                                         |
| 路由       | Next.js 语言段路由；交互层暂用 React Router v6 兼容                      |
...
| 部署       | Vercel，Next.js Server Rendering                                         |
...
├── public/                       # 直接静态资源
├── src/
│   ├── app/                      # Next.js App Router、Metadata Route 与语言布局
│   ├── App.tsx                   # React Router 兼容路由树
│   ├── proxy.ts                  # 语言检测、Cookie 与语言前缀重定向
│   ├── index.css                 # 全局样式入口，导入 styles/index.css
│   ├── assets/                   # 由 Next.js 打包的资源
...
│   ├── views/                    # 兼容层的路由页面组件；不要使用保留目录 src/pages
...
├── next.config.ts                # Next.js 构建与生产日志配置
...
- 兼容层页面放在 `src/views/`，并在 `src/App.tsx` 的嵌套路由中注册；新增可独立迁移的页面优先使用 `src/app/`。
...
- 开发时避免遗留调试日志。生产构建会通过 `next.config.ts` 的 `compiler.removeConsole` 删除除 `console.error` 和 `console.warn` 之外的 `console` 调用。
...
- API 基地址从 `NEXT_PUBLIC_API_BASE_URL` 读取，未设置时回退到 `https://lazer-api.g0v0.top`。不要在业务模块中复制硬编码基地址。
...
- 不要新增、提交或在客户端暴露真实 access token、服务端密钥或其他机密，`NEXT_PUBLIC_` 前缀的环境变量会进入浏览器包。
...
- `src/app/[lang]/[[...slug]]/page.tsx` 是当前 SSR 入口，负责元数据、结构化数据和用户页/谱面搜索页预取。
- `NextApplication` 在服务端使用 `MemoryRouter`，在浏览器使用带语言 `basename` 的 `BrowserRouter`；两端首屏输出必须保持一致。
...
- 修改 provider 树、语言路由或预取页面时，应同步检查 `NextApplication`、App Router 元数据、sitemap 与 proxy 行为。
...
| 变量                             | 用途                          | 作用域                |
| -------------------------------- | ----------------------------- | --------------------- |
| `NEXT_PUBLIC_API_BASE_URL`       | API 服务基地址                | 浏览器与 SSR 构建可见 |
| `NEXT_PUBLIC_SITE_URL`           | SEO 绝对 URL 基地址           | 浏览器与 SSR 构建可见 |
| `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | Cloudflare Turnstile 站点 key | 浏览器可见            |
| `BEATMAPSETS_SSR_ACCESS_TOKEN`   | 谱面集 SSR 请求认证 token     | 仅服务端环境          |

- `.env.example` 只放可公开的模板值；`.env`、`.env.local` 和任何真实凭据不得提交。
- Vercel 直接识别 Next.js；不要重新加入旧的 `vercel.json` SPA rewrite 或 `api/render.js`。
- 生产构建会生成 `.next/` 产物，不要提交构建产物。
