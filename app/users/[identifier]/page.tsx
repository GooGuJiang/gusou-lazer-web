...
export default async function UserProfilePage({ params, searchParams }: UserProfilePageProps) {
  ...
  const user = await getUserProfile(params.identifier, ruleset);
    ...
    <div className="space-y-8">
      <section className="rounded-3xl border border-white/10 bg-slate-900/60 p-8 shadow-xl shadow-sky-500/10">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link
              href={`/users/${params.identifier}?ruleset=${ruleset === 'osu' ? 'mania' : 'osu'}`}
      ...
      {stats ? (
        <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-6">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-white/60">社交</h3>
            <p className="mt-2 text-3xl font-bold text-white">关注者 {formatNumber(user.follower_count ?? 0)}</p>
            <p className="mt-1 text-sm text-white/60">帖子 {formatNumber(user.post_count ?? 0)}</p>
          </div>
        </section>
      ) : (
